...
function ImageTable({images, currentPage, onImageClick}: {currentPage: number, images: ImageData[], onImageClick: (url: string) => void}) { 
  const endOfImagesRef = useRef<HTMLDivElement | null>(null);
    

  const handleImageLoad = (isLastImage: boolean) => {
    if (isLastImage) {
  ...
  return (
    <div className="grid grid-cols-3 p-1 gap-0 md:grid-cols-5 bg-dark-blue">
      {images.map((image, index) => (
        <div 
          key={image.id} 
          className="bg-dark-blue rounded p-1"
          ref={index === images.length - 1 ? endOfImagesRef : null} // assign the ref to the last image
        >
          <div
          ...
          >
            <img
              ...
              onLoad={() => handleImageLoad(index === images.length - 1)}
            />
          </div>
        </div>
      ))}
...
const MainTable: React.FC = () => {
  ...
  const addImages = async (event: React.MouseEvent) => {
    ...
    // Scroll to the bottom of the page after images are added
